...
impl ServerManager {
    ...
    pub fn request_stop_server(&self, id: &str) -> Result<(), String> {
        ...
        std::thread::spawn(move || {
            ...
            if let Err(err) = manager.stop_server(&sid) {
                manager.append_log(&sid, &format!("[Sea Lantern CPE] 停止失败: {}", err));
    ...
    pub fn start_server(&self, id: &str) -> Result<(), String> {
        ...
        if settings.auto_accept_eula {
            ...
            let _ = std::fs::write(&eula, "# Auto-accepted by Sea Lantern CPE\neula=true\n");
        }
        ...
        let starter_installer_url = if startup_mode == "starter" {
            ...
            if let Some(sha256) = installer_sha256 {
                self.append_log(
                    id,
                    &format!(
                        "[Sea Lantern CPE] Starter 安装器: core={}, version={}, sha256={}",
        ...
        self.append_log(id, &format!("[Sea Lantern CPE] 启动命令: {}", command_for_log));
        ...
        self.save();
        self.append_log(id, "[Sea Lantern CPE] 服务器启动中...");
    ...
    pub fn stop_server(&self, id: &str) -> Result<(), String> {
        ...
        for _ in 0..20 {
            ...
            if let Some(child) = procs.get_mut(id) {
                ...
                self.clear_stopping(id);
                self.append_log(id, "[Sea Lantern CPE] 服务器已停止");
                return Ok(());
            }
        }
        ...
        if let Some(mut child) = procs.remove(id) {
            let _ = child.kill();
            let _ = child.wait();
            self.append_log(id, "[Sea Lantern CPE] 服务器超时，已强制终止");
        }
        self.clear_stopping(id);
        Ok(())
    }
